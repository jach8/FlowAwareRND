##### Heston Characteristic Function 3, with Bates Jump Extension + Flows and Exposure Weighting #####
...
def calibrate_heston_to_chain(strikes, market_prices, S0, tau, r, q=0.0, lambd_sv=0.0, alpha=1.5, N=2048, umax=100, weight_type='atm', flow_weights=['gamma', 'charm'], gexp=None, cexp=None, vexp=None, volume=None, oi=None, oi_chg=None):
    bounds = [
        (0.05, 5.0),    # kappa
        (0.01, 0.15),   # theta
        (0.01, 0.1),    # v0
    ...
    def objective(p):
        ...
        # Add flow/exposure weights if requested
        if flow_weights:
            abs_flow = True  # use absolute values for flow weights
            flow_weight = np.zeros_like(weights)
            if 'gamma' in flow_weights and gexp is not None:
                flow_weight += np.abs(gexp[valid]) if abs_flow else gexp[valid]  # abs or signed
            if 'charm' in flow_weights and cexp is not None:
                flow_weight += np.abs(cexp[valid]) if abs_flow else cexp[valid]
            if 'vanna' in flow_weights and vexp is not None:
                flow_weight += np.abs(vexp[valid]) if abs_flow else vexp[valid]
            if 'volume' in flow_weights and volume is not None:
                flow_weight += np.abs(volume[valid]) if abs_flow else volume[valid]
            if 'oi' in flow_weights and oi is not None:
                flow_weight += np.abs(oi[valid]) if abs_flow else oi[valid]
            if 'oi_chg' in flow_weights and oi_chg is not None:
                flow_weight += np.abs(oi_chg[valid]) if abs_flow else oi_chg[valid]
            
            flow_weight = flow_weight / (np.max(np.abs(flow_weight)) + 1e-6) + 0.1  # normalize + baseline
            weights *= flow_weight
            weights *= flow_weight
...
if __name__ == "__main__":
    ...
    oc_bs = oc_bs[oc_bs.gatherdate == gatherdates[-1]]
    ...
    gexp = gcdf['gexp'].values
    cexp = gcdf['cexp'].values
    vexp = gcdf['vexp'].values
    volume = gcdf['volume'].values
    oi = gcdf['openinterest'].values
    ...
    flow_weights = ['gamma', 'charm', 'vanna', 'volume', 'oi', 'oi_chg']
    ...
    opt_params, rmse = calibrate_heston_to_chain(strikes, market_prices, S0, tau, r, q, lambd_sv = lambd, alpha = alpha, N = N, umax = umax, weight_type = weight_type, flow_weights = flow_weights, gexp = gexp, cexp = cexp, vexp = vexp, volume = volume, oi = oi, oi_chg = oi_chg)
    ...
    print(f'IV RMSE: {iv_rmse:.4f}')

    #### Price RMSE Check
    price_rmse = np.sqrt(np.nanmean((model_prices - bs_model_prices)**2))
    print(f'Price RMSE: {price_rmse:.4f}')

    #### Price vs Market RMSE Check
    price_market_rmse = np.sqrt(np.nanmean((model_prices - market_prices)**2))
    print(f'Price vs Market RMSE: {price_market_rmse:.4f}')

    #### Price vs BS Model RMSE Check
    price_bs_model_rmse = np.sqrt(np.nanmean((model_prices - bs_model_prices)**2))
    print(f'Price vs BS Model RMSE: {price_bs_model_rmse:.4f}')
